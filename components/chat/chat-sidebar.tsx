...
import FilterModal from './chat-filter-modal' // We'll update this to be a Dialog
import { v4 as uuidv4 } from 'uuid';
import { FaFilter } from "react-icons/fa";
import {
  Search, MessageSquarePlus, Tags, Settings, Filter as FilterIcon, Pencil, Trash2, Users, Paperclip, X, Plus, ChevronDown, GripVertical
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchChats = async () => {
      ...
      try {
        ...
        const { data: labelAssignments, error: assignmentsError } = await supabase
        ...
        const chatDetailsPromises = chatIds.map(async (chatId: any) => {
          ...
          // Add labels to chat
          const chatLabels = labelAssignments
    ...
    <TooltipProvider delayDuration={100}>
      <div className="flex h-screen flex-col border-r bg-muted/40">
        ...
        <div className="flex h-16 flex-shrink-0 items-center justify-between border-b bg-background px-4">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="flex items-center gap-1 text-sm">
                ...
                {activeFilter && (
                  <X
                    className="ml-1 h-3 w-3 text-muted-foreground hover:text-foreground cursor-pointer" // Added cursor-pointer
                    onClick={(e) => { e.stopPropagation(); applyFilter(null); }}
                  />
                )}
                <ChevronDown className="h-3 w-3 opacity-50" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-60"> {/* Increased width slightly */}
              ...
              {definedFilters.map(f => (
                // Use DropdownMenuGroup to associate items, but interaction is per item
                <DropdownMenuGroup key={f.id}>
                  <div className="relative group/item flex items-center justify-between"> {/* Container for item and actions */}
                    <DropdownMenuItem
                      onClick={() => applyFilter(f)}
                      className={`flex-1 cursor-pointer pr-16 ${activeFilter?.id === f.id ? "bg-muted" : ""}`} // Add padding for actions
                    >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={(e) => { e.stopPropagation(); openEditFilterModal(f); }}
                          >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-destructive hover:text-destructive hover:bg-destructive/10" // Adjusted size & hover
          ...
          {/* ... rest of the header, search, and chat list ... */}
          <div className="flex items-center space-x-1">
            {[
              { id: 'new-chat', label: 'New Chat', icon: IoCreate, action: handleNewChat },
              { id: 'labels', label: 'Manage Labels', icon: Settings, href: '/labels' },
            ].map(item => (
              <Tooltip key={item.id}>
                <TooltipTrigger asChild>
                  {item.action ? (
          ...
          {loading ? (
            ...
            <ul className="divide-y">
              {filteredChats.map((chat) => {
                ...
                const avatarFallback = chat.is_group ? chatName.substring(0, 1).toUpperCase() : (chatName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || '??');
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <Avatar className="h-10 w-10 flex-shrink-0 border">
                            ...
                            <AvatarFallback className={chat.is_group ? 'bg-sky-100 text-sky-600 dark:bg-sky-700 dark:text-sky-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-1 pl-[calc(2.5rem+0.75rem)]"> 
                          ...
                          {chat.labels.length > 3 && (
                            <Badge variant="outline" className="py-0.5 px-1.5 text-xs font-normal">+{chat.labels.length - 3}</Badge>
                          )}
                        </div>
                      )}
                        </div>
