...
export default function FilterModal({ isOpen, onClose, onSave, existingFilter, userLabels }: FilterModalProps) {
  ...
  const handleSubmit = () => {
    if (!name.trim()) { setError("Filter name cannot be empty."); return; }
    ...
    if (Object.keys(finalCriteria).length === 0 && !name.trim().toLowerCase().includes("all chats")) { // Allow "All Chats" filter with no criteria
      setError("Please define at least one filter criterion or name it 'All Chats'."); return;
    }
    setError(null);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure the criteria for your custom chat filter.
          </DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-[60vh] pr-4">
            ...
            <div>
                <Label htmlFor="filter-modal-name">Filter Name <span className="text-destructive">*</span></Label>
                <Input 
                id="filter-modal-name" 
                value={name} 
            ...
            <fieldset className="space-y-4 rounded-md border p-4 pt-2">
                ...
                <Label htmlFor="modal-unread" className="font-normal select-none">Unread Messages Only</Label>
                </div>
                ...
                <Input id="modal-chat_name_contains" value={currentCriteria.chat_name_contains || ''} onChange={e => handleCriteriaChange('chat_name_contains', e.target.value)} className="mt-1" placeholder="e.g., Project Alpha" />
                </div>
                ...
                {(currentCriteria.is_group === false || currentCriteria.is_group === undefined) && (
                    <div>
                        ...
                        <Input id="modal-other_participant_name_contains" value={currentCriteria.other_participant_name_contains || ''} onChange={e => handleCriteriaChange('other_participant_name_contains', e.target.value)} className="mt-1" placeholder="e.g., John Doe"/>
                    </div>
                )}
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="modal-has_attachments" className="font-normal select-none">Has Attachments (in last message)</Label>
                </div>
                ...
                <div className="pt-2">
                    ...
                    {userLabels.map(label => (
                        ...
                        <Label htmlFor={`modal-label-${label.id}`} className="flex items-center font-normal select-none">
                            <span className="mr-2 h-3 w-3 rounded-full" style={{backgroundColor: label.color}}></span>{label.name}
                        </Label>
                        </div>
                    ))}
                    </div>
